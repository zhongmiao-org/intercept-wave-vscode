...
export interface WsRuleModalProps {
    ...
    labels: {
        ...
        timelineInvalidAtMs: string;
        jsonFormatError: string;
        selectTimelineItem: string;
    };
}

export function WsRuleModal({
    open,
    draft,
    onChange,
    onSave,
    ...
    const commitTimelineEdit = () => {
        ...
        if (!Number.isFinite(atMs) || atMs < 0) {
            window.alert(labels.timelineInvalidAtMs);
            return;
        }
        ...
        if (timelineEditMode === 'add') {
            ...
            // 尝试优先选中时间点相同的新项
            nextSelected = next.findIndex(
                it => it.atMs === item.atMs && it.message === item.message
            );
            if (nextSelected < 0) {
                nextSelected = next.indexOf(item);
            }
    ...
    const handleFormatMessage = () => {
        ...
        try {
            ...
            window.alert(labels.jsonFormatError.replace('{0}', msg));
        }
    };
            ...
            >
                        ...
                        <>
                            ...
                            <div style={{ gridColumn: '1 / span 2' }}>
                                ...
                                <div style={{ display: 'flex', gap: 8, marginTop: 6 }}>
                                    <button onClick={beginAddTimelineItem}>
                                        {labels.timelineAdd}
                                    </button>
                                    <button
                                        onClick={beginEditTimelineItem}
                                        disabled={selectedTimelineIndex == null}
                                    >
                                        {labels.timelineEdit}
                                    </button>
                                    <button
                                        onClick={deleteTimelineItem}
                                        disabled={selectedTimelineIndex == null}
                                    >
                                    ...
                                    >
                                        ...
                                        >
                                            ...
                                            <textarea
                                                ...
                                                onChange={e =>
                                                    setTimelineDraftMessage(e.target.value)
                                                }
                                            />
                                        </div>
                                        <div
                                        ...
                                        >
                                            <button onClick={cancelTimelineEdit}>
                                                {labels.timelineEditorCancel}
                                            </button>
                                            <button onClick={commitTimelineEdit}>
                                                {labels.timelineEditorSave}
                                            </button>
                                        </div>
                                    </div>
                                )}
                ...
                <div style={{ marginTop: 8 }}>
                    ...
                    <textarea
                        ...
                        placeholder={isTimelineMode ? labels.selectTimelineItem : ''}
                    />
                </div>
